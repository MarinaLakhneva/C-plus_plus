...
public:
    ...
    Wrapper(Subject *subj, 
            Type(Subject::*fun)(Args...), 
            const std::vector<std::pair<std::string, Type>> argsList) {
...
private:
    ...
    void initializeFun(Subject *subj, Type(Subject::*fun)(Args...)){
        ...
        {
            return Fun(subj, fun, argsForFun, std::make_index_sequence<sizeof...(Args)>{});
        };
    }

    template<typename Subject, typename Name, size_t... Indexis>
    Type Fun(Subject* subj, Name functionName, const std::vector<Type>& argsList, std::index_sequence<Indexis...>) {
...
public:
    ...
    void register_ñommand(Wrapper<Type> *wrapper, std::string functionName) {
