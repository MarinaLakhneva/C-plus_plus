...
bool Calculator::processOperationError(int *index) {
    ...
    while (!isalpha(str_exp[currentIndex]) 
        && !isdigit(str_exp[currentIndex])
        && str_exp[currentIndex] != BaseOperation::left_bracket 
        && str_exp[currentIndex] != BaseOperation::right_bracket 
        && str_exp[currentIndex] != BaseOperation::unary_minus) {
        currentIndex++;
